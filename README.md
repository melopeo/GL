...
## Usage:
Let `Wpos,Wneg` are adjacency matrices of positive and negative graphs, `supervised_nodes_idx` is an array with indexes of labeled nodes, `labels_of_supervised_nodes` is an array with the corresponding labels, `Laplacian_str` is a string indicating with signed Laplacian to use, and `numEigenvectors` is a scalar indicating how many eigenvectors to take. 
